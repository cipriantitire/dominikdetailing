...
export default function SiteHeader() {
  ...
  return (
    <>
      {/* Top bar */}
      <div className="border-b border-white/[0.04] bg-[#07070a] text-[11px] tracking-wide text-[#686878] uppercase">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-2.5 md:px-6">
          <div className="flex items-center gap-5">
            <a
              href={siteConfig.phoneHref}
              className="hidden items-center gap-1.5 transition hover:text-[#9696a3] focus-visible:ring-1 focus-visible:ring-[#1d4ed8]/40 outline-none rounded md:flex"
            >
              <Phone size={12} />
          ...
          <nav className="hidden items-center gap-8 md:flex">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-[13px] font-medium text-[#9696a3] transition hover:text-white focus-visible:ring-1 focus-visible:ring-[#1d4ed8]/40 outline-none rounded"
              >
                {link.label}
              </Link>
            ))}
